...
for(f in 1:n_predictions){
  ...
  for(c in c("max")){
    ...
    forest <- parallel_build_random_forest(train,treatment_list,response,control,n_trees = 100,n_features = 3,
                                           criterion = c,remain_cores = 10)
    pred <- predict_forest_df(forest,test)
    write.csv(pred, paste("Predictions/Test/Hillstrom/random_forest_",c,"new",as.character(f),".csv",sep = ""), row.names = FALSE)
  }
  end_time <- Sys.time()
...
start_time <- Sys.time()
folder <- "Predictions/Test/Hillstrom/"
outcomes <- c()
n_predictions <- 20
p_treated <- c()
